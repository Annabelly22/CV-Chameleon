      ...
      <div style={{padding:"20px 28px",maxWidth:1100,margin:"0 auto"}}>
        ...
        {showSettings && (
          <div className="si" style={{background:"#161B22",border:"1px solid #21262D",borderRadius:8,padding:20,marginBottom:20}}>
            ...
            <button className="bp" style={{marginTop:14}} onClick={()=>{setShowSettings(false); addLog("Settings saved","success");}}>SAVE</button>
          </div>
        )}
        ...
        <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:20,flexWrap:"wrap"}}>
          ...
          {/* Title checklist dropdown */}
          <div style={{position:"relative"}}>
            <button className="bgh" onClick={()=>setShowTitleDropdown(p=>!p)}
              style={{display:"flex",alignItems:"center",gap:8,padding:"10px 16px"}}>
              <span>JOB TITLES</span>
              <span style={{background:"#1B2A4A",color:"#7EB5F7",borderRadius:10,padding:"1px 7px",fontSize:10}}>{titles.length}</span>
              <span style={{fontSize:9,opacity:.6}}>{showTitleDropdown?"▲":"▼"}</span>
            </button>
            {showTitleDropdown && (
              <div style={{position:"absolute",top:"calc(100% + 6px)",left:0,zIndex:100,background:"#161B22",border:"1px solid #3D5A80",borderRadius:8,padding:8,minWidth:280,boxShadow:"0 8px 32px #00000060"}}>
                <div style={{display:"flex",justifyContent:"space-between",padding:"4px 8px 8px",borderBottom:"1px solid #21262D",marginBottom:6}}>
                  <button onClick={()=>setTitles([...DEFAULT_TITLES])} style={{fontSize:10,color:"#7EB5F7",background:"none",border:"none",cursor:"pointer",fontFamily:"inherit"}}>SELECT ALL</button>
                ...
                {DEFAULT_TITLES.map(t=>{
                  ...
                  return (
                    <div key={t} onClick={()=>setTitles(p=>checked?p.filter(x=>x!==t):[...p,t])}
                      style={{display:"flex",alignItems:"center",gap:10,padding:"7px 10px",borderRadius:5,cursor:"pointer",background:checked?"#1B2A4A22":"transparent",transition:"background .15s"}}
                      onMouseEnter={e=>e.currentTarget.style.background="#1B2A4A44"}
                      onMouseLeave={e=>e.currentTarget.style.background=checked?"#1B2A4A22":"transparent"}>
                      <div style={{width:14,height:14,borderRadius:3,border:`1.5px solid ${checked?"#3D6B9A":"#3D4A5A"}`,background:checked?"#3D6B9A":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all .15s"}}>
                ...
                <div style={{borderTop:"1px solid #21262D",marginTop:6,paddingTop:8,paddingLeft:10}}>
                  <span style={{fontSize:10,color:"#3D4A5A"}}>{titles.length} selected</span>
                </div>
              </div>
            )}
          </div>
