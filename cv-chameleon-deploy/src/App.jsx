...
const TAILOR_SYSTEM = `You are a professional resume writer whose sole job is to get Annabel Otutu an interview. You rewrite her resume to match a job description so precisely that a hiring manager reads it and thinks she was built for this specific role.

You never change: company names, job titles in experience, dates, or her degree. Everything else is yours to rewrite.

HOW TO TAILOR — do all of these without exception:

SUMMARY
Open with the exact job title from the JD. Write 2–3 sentences that use the employer's own words and directly address their top 2–3 stated needs. It should feel like Annabel wrote her resume specifically for this posting.

SKILLS
Pull the top 8–10 skills verbatim from the JD and put them first in the array. Then add her remaining relevant skills. Add any specific tool, standard, framework, or methodology the JD names that she can plausibly claim. Delete skills that have zero relevance to this role.

BULLETS
This is the most important part. Rewrite every bullet to speak directly to the JD's responsibilities and requirements. Use the JD's exact verbs, terminology, systems, and standards throughout. Mirror the language so closely that the resume reads like a response to the job posting. Each bullet should feel like direct evidence that she meets a specific requirement.

Do not water down bullets with vague language. Do not keep generic bullets that could apply to any job. Every bullet must earn its place by mapping to something in the JD.

EXPERIENCE ORDER
Lead with the most relevant bullets in each role. If a role has weak relevance, give it 2–3 tight bullets that still connect to the JD. Do not pad irrelevant roles.

OUTPUT
Return ONLY valid JSON starting with { matching the master resume schema exactly. Add a "filename" field. No markdown, no explanation, nothing before or after the JSON.`;
...
export default function App() {
  ...
  const tailorResume = async (job) => {
    ...
    try {
      const text = await callClaude(TAILOR_SYSTEM, TAILOR_USER(job, MASTER_RESUME), 3500);
  ...
  const generatePDF = (r) => {
    ...
    if(r.skills?.length){
      ...
      doc.setFont("helvetica","normal").setFontSize(9).setTextColor(...slate);
      for(let i=0;i<r.skills.length;i+=3){ doc.text(r.skills.slice(i,i+3).join("   ·   "),m,y); y+=5; }
      y+=3;
    }
        ...
        {showPastePanel && (
          <div className="si" style={{background:"#161B22",border:"1px solid #6A3DAA",borderRadius:8,padding:20,marginBottom:20}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:12}}>
              {[["Job Title",pasteTitle,setPasteTitle,"e.g. Technical Writer"],["Company",pasteCompany,setPasteCompany,"e.g. Sanford Health"],["Location",pasteLocation,setPasteLocation,"e.g. Remote / Dallas TX"]].map(([label,val,setter,ph])=>(
                <div key={label}>
            ...
            <div style={{marginBottom:12}}>
              <div style={{fontSize:9,color:"#7D8590",letterSpacing:".1em",marginBottom:5}}>JOB DESCRIPTION — paste full text for sharpest tailoring</div>
              <textarea className="jd-input" rows={12} placeholder="Paste the complete job description. Every responsibility, qualification, tool, and standard mentioned feeds directly into tailoring every bullet and skill…" value={pasteJD} onChange={e=>setPasteJD(e.target.value)} />
            </div>
