...
const TITLE_GROUPS = [
  { label: "Highest Probability (80–95%)", color: "#4ADE80", titles: [
    "Technical Writer","Technical Writer II","Documentation Specialist","Content Developer",
    "Content Strategist","Knowledge Management Specialist","Technical Communications Specialist",
    "User Experience Writer","API Documentation Writer","Software Documentation Writer"
  ]},
  { label: "IT Support & Analysis", color: "#7EB5F7", titles: [
    "IT Support Specialist","Help Desk Analyst II","Help Desk Analyst III",
    "System Documentation Analyst","IT Business Analyst","Technical Support Specialist",
    "Desktop Support Technician","Network Support Technician"
  ]},
  { label: "Federal & Government Contractor", color: "#C084FC", titles: [
    "Technical Writer (Cleared)","Documentation Specialist (Federal)",
    "IT Support Analyst (Government)","Cybersecurity Documentation Specialist",
    "Compliance Documentation Specialist"
  ]},
  { label: "Cybersecurity — Entry Level (60–80%)", color: "#F59E0B", titles: [
    "Cybersecurity Analyst I","SOC Analyst I","Information Security Analyst",
    "Vulnerability Assessment Analyst","Compliance Analyst","Risk Assessment Analyst",
    "Security Documentation Specialist"
  ]},
  { label: "Business Analysis", color: "#F59E0B", titles: [
    "Junior Business Analyst","Process Documentation Analyst","Business Process Analyst",
    "Requirements Analyst","Quality Assurance Analyst","Training Developer"
  ]},
  { label: "Project Coordination", color: "#F59E0B", titles: [
    "Technical Project Coordinator","Documentation Project Manager",
    "IT Project Coordinator","Training Coordinator"
  ]},
  { label: "Software & Tech (40–60%)", color: "#FB923C", titles: [
    "Product Documentation Manager","Developer Relations Specialist",
    "Training Content Developer","Customer Success Specialist (Technical)",
    "Technical Proposal Manager","Technical Account Manager","Solutions Specialist"
  ]},
  { label: "Healthcare & Finance", color: "#FB923C", titles: [
    "Compliance Documentation Specialist","Policy and Procedure Writer",
    "Training Material Developer","Process Improvement Analyst"
  ]},
  { label: "Consulting", color: "#FB923C", titles: [
    "Technology Consultant","Implementation Specialist","Training Consultant",
    "Contracts Manager","Commercial Project Manager","Procurement Project Manager"
  ]},
  { label: "Hybrid Sweet Spot Roles", color: "#F472B6", titles: [
    "RFP Manager","Proposal Manager","Implementation Project Manager",
    "Contracts Project Manager","Strategic Sourcing Project Manager",
    "Technical Program Manager (TPM)","Documentation Manager",
    "Knowledge Management Lead","Legal Operations Manager","RFP / Proposal Manager"
  ]}
];
...
const LOCATION_OPTIONS = [
  { label: "Austin", emoji: "🔥" },
  { label: "Houston", emoji: "💕" },
  { label: "Dallas", emoji: "❤️" },
  { label: "Remote", emoji: "😌" },
  { label: "Contract", emoji: "🤑" },
  { label: "Government", emoji: "💫" },
  { label: "BuiltIn", emoji: "🤌🏽" },
];
...
export default function App() {
  ...
  const findJobs = async () => {
    ...
    addLog("Searching real job boards…", "loading");
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        // If JSearch not configured, fall back to Claude-generated listings
        if (data?.error?.includes("SERPAPI_KEY")) {
          addLog("JSearch not configured — generating via Claude…", "info");
          await findJobsViaClaudeFallback();
          return;
        }
        throw new Error(data?.error || `Server error ${res.status}`);
      }
      if (!data.jobs?.length) {
        addLog("No results from job boards — generating via Claude…", "info");
        await findJobsViaClaudeFallback();
        return;
      }
      setJobs(data.jobs);
      addLog(`${data.jobs.length} real jobs found`, "success");
      setPhase("results");
    } catch (e) {
  ...
  const findJobsViaClaudeFallback = async () => {
    ...
    try {
      const text = await callClaude(
        ...
        `Generate 10 realistic job postings for: ${titlesStr}\nLocations: ${locsStr}\n\nEach object needs: id, title, company, location, url, companyUrl, source (LinkedIn/Indeed/Builtin/Company Site), description (3-4 sentences), salary (realistic range or null). Mix company types: healthcare, defense, tech, consulting, govcon. Return ONLY the JSON array.`,
        3000
      );
      const parsed = extractJSON(text);
      if (!Array.isArray(parsed) || !parsed.length) throw new Error("No jobs returned");
      setJobs(parsed.map((j, i) => ({ ...j, id: j.id || `job_${i}` })));
      addLog(`${parsed.length} jobs generated (add JSEARCH_API_KEY for live results)`, "success");
      setPhase("results");
    } catch (e) {
      addLog(`${e.message}`, "error");
      setPhase("idle");
      ...
      <div style={{padding:"20px 28px",maxWidth:1100,margin:"0 auto"}}>
        ...
        {showSettings && (
          <div className="si" style={{background:"#161B22",border:"1px solid #21262D",borderRadius:8,padding:20,marginBottom:20}}>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:13,fontWeight:700,color:"#7EB5F7",marginBottom:10}}>SEARCH SETTINGS</div>
            <div style={{fontSize:11,color:"#7D8590",lineHeight:1.8}}>
              Use the <span style={{color:"#E6EDF3"}}>SEARCH FILTERS</span> dropdown to select job titles (max 10) and target locations.<br/>
              The app generates 10–12 realistic job postings matching your selections via Claude.
            </div>
            <button className="bp" style={{marginTop:14}} onClick={()=>{setShowSettings(false);}}>CLOSE</button>
          </div>
        )}
        ...
        <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:20,flexWrap:"wrap"}}>
          ...
          {/* Title + Location checklist dropdown */}
          <div style={{position:"relative"}}>
            <button className="bgh" onClick={()=>setShowTitleDropdown(p=>!p)}
            ...
            {showTitleDropdown && (
              <div style={{position:"absolute",top:"calc(100% + 6px)",left:0,zIndex:100,background:"#161B22",border:"1px solid #3D5A80",borderRadius:8,width:420,maxHeight:520,overflowY:"auto",boxShadow:"0 8px 40px #00000080"}}>
                ...
                <div style={{padding:"12px 14px 8px",borderBottom:"1px solid #21262D",position:"sticky",top:0,background:"#161B22",zIndex:2}}>
                  <div style={{fontSize:10,color:"#7D8590",letterSpacing:".1em",marginBottom:8}}>LOCATIONS (PRIORITY)</div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {LOCATION_OPTIONS.map(({label,emoji})=>{
                      const active = locations.includes(label);
                      return (
                        <button key={label} onClick={()=>setLocations(p=>active?p.filter(l=>l!==label):[...p,label])}
                          style={{background:active?"#1B2A4A":"transparent",border:`1px solid ${active?"#3D6B9A":"#2D3440"}`,borderRadius:20,padding:"4px 10px",fontSize:11,color:active?"#7EB5F7":"#7D8590",cursor:"pointer",fontFamily:"inherit",transition:"all .15s"}}>
                          {emoji} {label}
                        </button>
                      );
                    })}
                ...
                <div style={{padding:"6px 14px",borderBottom:"1px solid #21262D",display:"flex",justifyContent:"space-between",alignItems:"center",background:"#161B22",position:"sticky",top:72,zIndex:2}}>
                  <span style={{fontSize:10,color:titles.length>=10?"#F59E0B":"#3D4A5A",letterSpacing:".06em"}}>
                    {titles.length>=10?"⚠ MAX 10 SELECTED — deselect to change":"JOB TITLES  ·  SELECT UP TO 10"}
                  </span>
                ...
                {TITLE_GROUPS.map(group=>(
                  <div key={group.label}>
                    ...
                    {group.titles.map(t=>{
                      const checked = titles.includes(t);
                      const atMax = titles.length >= 10 && !checked;
                      return (
                        <div key={t} onClick={()=>{ if(atMax) return; setTitles(p=>checked?p.filter(x=>x!==t):[...p,t]); }}
                          style={{display:"flex",alignItems:"center",gap:10,padding:"6px 14px",cursor:atMax?"not-allowed":"pointer",opacity:atMax?.35:1,transition:"background .1s"}}
                          onMouseEnter={e=>{if(!atMax)e.currentTarget.style.background="#1B2A4A33"}}
                          onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                          <div style={{width:13,height:13,borderRadius:3,border:`1.5px solid ${checked?group.color:"#3D4A5A"}`,background:checked?group.color:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all .15s"}}>
                            {checked&&<span style={{color:"#0D1117",fontSize:9,fontWeight:700,lineHeight:1}}>✓</span>}
                          </div>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"flex-start"}}>
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:"flex",gap:10,marginBottom:6,flexWrap:"wrap"}}>
                          {job.salary&&<span style={{fontSize:10,color:"#4ADE80",letterSpacing:".04em"}}>💰 {job.salary}</span>}
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:8,minWidth:120,alignItems:"flex-end"}}>
                        ...
                        <a href={job.url} target="_blank" rel="noopener noreferrer"
                          style={{fontSize:10,color:"#3D6B9A",textDecoration:"none",textTransform:"uppercase",letterSpacing:".05em"}}>APPLY →</a>
                        {job.applyOptions?.length > 1 && job.applyOptions.slice(1).map((opt,i)=>(
                          <a key={i} href={opt.url} target="_blank" rel="noopener noreferrer"
                            style={{fontSize:10,color:"#2D4A6A",textDecoration:"none",textTransform:"uppercase",letterSpacing:".05em"}}>{opt.label} →</a>
                        ))}
                        <a href={job.companyUrl} target="_blank" rel="noopener noreferrer"
                          style={{fontSize:10,color:"#2D4A5A",textDecoration:"none",textTransform:"uppercase",letterSpacing:".05em"}}>COMPANY →</a>
                      </div>
                    </div>
                    {done&&(
